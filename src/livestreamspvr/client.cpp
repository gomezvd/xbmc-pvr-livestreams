...
PVRLiveStreamsData *m_data = NULL;
bool m_bIsPlaying = false;
PVRLiveStreamsChannel m_currentChannel;
...
extern "C" {
  ...
  void ADDON_ReadSettings(void)
  {
    char buffer[1024];
    int iPathType = 0;
    if (!XBMC->GetSetting("xmlPathType", &iPathType))
    {
      iPathType = 1;
    }
    if (iPathType)
    {
      if (XBMC->GetSetting("xmlUrl", &buffer))
      {
        g_strXMLPath = buffer;
      }
      if (!XBMC->GetSetting("xmlCache", &g_bCacheXML))
      {
        g_bCacheXML = true;
      }
    }
    ...
    {
      if (XBMC->GetSetting("xmlPath", &buffer))
      {
        g_strXMLPath = buffer;
      }
      g_bCacheXML = false;
    ...
    if (g_strXMLPath == "")
    {
      g_strXMLPath = GetClientFilePath(XML_FILE_NAME);
    }
  ...
  ADDON_STATUS ADDON_SetSetting(const char *settingName, const void *settingValue)
  {
    ...
    string strFile = GetUserFilePath(XML_FILE_NAME);
  ...
  {
    static const char *strBackendName = "LiveStreams PVR Add-on";
  ...
  {
    snprintf(signalStatus.strAdapterName, sizeof(signalStatus.strAdapterName), "LiveStreams Adapter 1");
  ...
  {
    if (m_data)
      return m_data->GetChannelURL(channel);
